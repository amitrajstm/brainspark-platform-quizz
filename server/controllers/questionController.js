...
exports.createBulkQuestions = async (req, res) => {
  try {
    ...
    for (const q of questions) {
      if (!q.quizId || !q.questionText || !Array.isArray(q.options)) {
        return res.status(400).json({
          success: false,
          error: "Each question must include quizId, questionText, and options.",
        });
      }
    ...
    console.error("ERROR CREATING BULK QUESTIONS:", e);
    return res.status(500).json({
      success: false,
      error: "Internal server error",
    });
